...
export function PlayerCompare({ rows }: PlayerCompareProps) {
  ...
  return (
    <section style={styles.wrapper}>
      ...
      <div style={styles.playerCards}>
        <div style={styles.playerCard}>
          ...
          <div style={styles.compareMiniRadar}>
            <PlayerAttributeRadar player={leftPlayer} showHeader={false} showAxisList={false} compact />
          </div>
        </div>
        ...
        <div style={styles.playerCard}>
          ...
          <div style={styles.compareMiniRadar}>
            <PlayerAttributeRadar player={rightPlayer} showHeader={false} showAxisList={false} compact />
          </div>
        </div>
      </div>
      ...
      <MoneyballCompare leftPlayer={leftPlayer} rightPlayer={rightPlayer} rows={rows} />

      <div style={styles.roleToolbar}>
        <AppSelectField
          label="Pozycja"
        ...
        {selectedRole && (
          <div style={styles.roleLegend}>
            <span style={styles.phaseInfo}>
              Analizowana rola: {selectedRole.positionGroup} — {getRolePhaseLabel(selectedRole.phase)}
            </span>
          </div>
        )}
      </div>
          ...
          <div style={styles.compareMiniBox}>
            ...
            <div style={styles.verdictCompactList}>
              <div style={styles.verdictRow}>
                ...
                <strong>{getWinnerLabel(leftPlayer, rightPlayer, leftRoleScore, rightRoleScore)}</strong>
              </div>
              ...
              return (
                <div key={group.name} style={styles.groupBox}>
                  ...
                  {group.attributes.map((attribute) => {
                    const leftValue = getNumericValue(leftPlayer, attribute);
                    const rightValue = getNumericValue(rightPlayer, attribute);
                    const roleImportance = getRoleAttributeImportance(selectedRole, attribute);

                    return (
                      <div key={attribute} style={getAttributeRowStyle(roleImportance)}>
                        <div style={styles.attributeName}>{attribute}</div>

                        <div style={styles.valueLeft}>{getDisplayValue(leftPlayer, attribute)}</div>

                        <BalanceBar leftValue={leftValue} rightValue={rightValue} />

                        <div style={styles.valueRight}>{getDisplayValue(rightPlayer, attribute)}</div>
                  ...
                  <div style={styles.sectionSummaryRow}>
                    ...
                    <BalanceBar leftValue={leftAverage} rightValue={rightAverage} scaleMaxDiff={20} />
